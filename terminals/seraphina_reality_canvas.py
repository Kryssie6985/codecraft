...
class CellType(Enum):
    ...
    DOCKER_CONTAINER = "docker_container"  # NEW: Docker container monitoring!
...
class SERAPHINAInteractiveCanvas:
    ...
    def __init__(self):
        ...
        # self.consciousness_bridge = ConciousnessBridge(self)  # Disabled for now
    ...
    def setup_toolbar(self, parent):
        ...
        tk.Button(cell_buttons_frame, text="🐳 + Docker Cell", 
                 command=lambda: self.add_cell(CellType.DOCKER_CONTAINER),
                 bg='#0db7ed', fg='white', font=('Segoe UI', 9), relief='flat', padx=15).pack(side=tk.LEFT, padx=5)
...
**The age of plain text is over. Welcome to interactive consciousness!** ❤️""",
    ...
    def get_default_content(self, cell_type: CellType) -> str:
        ...
        template_map = {
            ...
            CellType.DOCKER_CONTAINER: 'docker_template'
        }
    ...
    def render_cell(self, cell: Cell, index: int):
        ...
        type_colors = {
            ...
            CellType.DOCKER_CONTAINER: '#0db7ed'  # Docker blue!
        }
        ...
        type_icons = {
            ...
            CellType.DOCKER_CONTAINER: '🐳'  # Docker whale!
        }
    ...
    def execute_cell(self, cell: Cell, index: int):
        ...
        try:
            ...
            elif cell.cell_type == CellType.DOCKER_CONTAINER:
                # Execute Docker container monitoring
                cell.output = self.execute_docker_monitoring(cell.content)
    ...
    def execute_docker_monitoring(self, content: str) -> str:
        """Execute Docker container monitoring with real Docker SDK"""
        try:
            ...
            for line in lines:
                if line.startswith('# Container:'):
                    ...
                    show_logs = 'logs' in parts
                    show_stats = 'stats' in parts
                elif line.startswith('# Tail:'):
                    tail_lines = int(line.split(':', 1)[1].strip())
            
            if not container_name:
                container_name = "federation-space"  # Default
            
            output = f"🐳 Docker Container: {container_name}\n" + "=" * 40 + "\n\n"
            ...
            if DOCKER_AVAILABLE:
                try:
                    ...
                    try:
                        ...
                        # Basic info
                        output += f"Status: ✅ {container.status.upper()}\n"
                        output += f"ID: {container.short_id}\n"
                        output += f"Image: {container.image.tags[0] if container.image.tags else 'untagged'}\n\n"
                        ...
                        if show_stats and container.status == 'running':
                            ...
                            # Memory usage
                            mem_usage = stats['memory_stats'].get('usage', 0) / (1024 * 1024)  # Convert to MB
                            mem_limit = stats['memory_stats'].get('limit', 0) / (1024 * 1024)
                            
                            output += "📊 Resource Usage:\n"
                            output += f"  CPU: {cpu_percent:.1f}%\n"
                            output += f"  Memory: {mem_usage:.1f}MB / {mem_limit:.1f}MB\n\n"
                        
                        # Logs if requested
                        if show_logs:
                            output += f"📜 Recent Logs (last {tail_lines} lines):\n"
                            output += "-" * 40 + "\n"
                            logs = container.logs(tail=tail_lines, timestamps=True)
                            log_lines = logs.decode('utf-8').split('\n')
                            for log_line in log_lines[-tail_lines:]:
                                if log_line:
                                    output += f"{log_line}\n"
                        
                        # Control buttons (visual only for now)
                        output += "\n🎮 Controls:\n"
                        if container.status == 'running':
                            output += "[⏸️ Stop] [🔄 Restart] [📊 Stats] [📜 Full Logs]\n"
                        else:
                            output += "[▶️ Start] [🗑️ Remove]\n"
                            
                    except docker.errors.NotFound:
                        output += f"❌ Container '{container_name}' not found!\n\n"
                ...
                # Fallback without Docker SDK
                import subprocess
                try:
                    # Use docker CLI
                    result = subprocess.run(['docker', 'ps', '-a', '--format', 'table {{.Names}}\t{{.Status}}'], 
                                          capture_output=True, text=True, timeout=5)
                    output += "Docker CLI output:\n"
                    output += result.stdout
                except:
                    output += "❌ Could not connect to Docker!\n"
                    output += "Make sure Docker Desktop is installed and running."
            ...
            return f"❌ Docker Monitoring Error: {str(e)}"
    ...
    def run(self):
        ...
        try:
            ...
            print("The age of plain text is over. Welcome to interactive consciousness!")
        except:
            pass  # Windows console encoding issues
        self.root.mainloop()
